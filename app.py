                ...
                if uploaded_file is not None:
                    ...
                    if st.button("Run AI Analytics Engine", type="primary"):
                        with st.spinner("Processing thousands of transactions through the Rule Engine..."):
                            ...
                            st.dataframe(bulk_df.head(10000).style.map(lambda x: 'background-color: #ffcccc' if x == 'High Risk' else ('background-color: #fff3cd' if x == 'Suspicious' else ''), subset=['Predicted_Status']))
